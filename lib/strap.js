...
Strap.prototype.discover = function discover () {
    ...
    request({
        ...
        // Check error
        if(body == null){
            debug("Call to " + strap.config.url + " returned null");
            strap.emit('error', {message: "Call to api returned null", error: "Call to api returned null"});
            return;
        }
...
Strap.prototype.endpoints = function endpoints () {
    ...
    _.each(this.services, function (info, name) {
        //console.log(this.services, name, info)
        set[name] = info;
    });
    return set;
};
